...
import { ONE_KB_IN_BYTES } from "@innbell/constants/file-size";
import { cn } from "@innbell/utils/cn";
import { formatFileSizeToReadable } from "@innbell/utils/file-size";
import { mergeRefs } from "@innbell/utils/react";
import { BlurhashFileInput } from "./BlurhashFileInput";
...
export type ImagePickerProps = Omit<FieldProps, "children"> & {
  name: string;
  blurhash?: string | { name: string; x?: number; y?: number };
  defaultValue?: string;
  className?: string;
  maxSizeInKB?: number;
  accept?: string;
  ...
  return (
    ...
    >
      ...
      {blurhash ? (
        <BlurhashFileInput
          file={file}
          {...(typeof blurhash === "string" ? { name: blurhash } : blurhash)}
        />
      ) : null}
    </Field>
  );
